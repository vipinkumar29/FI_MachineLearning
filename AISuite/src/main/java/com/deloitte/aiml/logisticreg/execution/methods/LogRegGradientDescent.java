package com.deloitte.aiml.logisticreg.execution.methods;

import com.deloitte.aiml.logisticreg.web.controller.LogisticRegression;

import Jama.Matrix;

public class LogRegGradientDescent {
	...
	//n = number of features (including fake feature)
		...
		double currError = LogisticRegression.error(theta, X, y);
		if(PRINT_PROGRESS){
		...
		for (int i = 0; i < NUM_ITERATIONS; i++) {
			...
			for (int j = 0; j < n; j++) {
				...
				for (int row = 0; row < m; row++) {
					deriv += (LogisticRegression.predict(theta, X.getMatrix(row, row, 0, n-1)) - y.getArray()[row][0]) * X.getArray()[row][j];
				}
			...
			currError = LogisticRegression.error(theta, X, y);
			if(PRINT_PROGRESS){
