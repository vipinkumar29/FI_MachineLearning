package com.deloitte.aiml.linearreg.execution.methods;

import com.deloitte.aiml.linearreg.web.controller.LinearRegressionImpl;

import Jama.Matrix;

public class LinRegGradientDescent {
	...
	//n = number of features (including fake feature)
		...
		double currError = LinearRegressionImpl.error(theta, X, y);
		if(PRINT_PROGRESS){
		...
		for (int i = 0; i < NUM_ITERATIONS; i++) {
			...
			for (int j = 0; j < n; j++) {
				...
				for (int row = 0; row < m; row++) {
					deriv += (LinearRegressionImpl.predict(theta, X.getMatrix(row, row, 0, n-1)) - y.getArray()[row][0]) * X.getArray()[row][j];
				}
			...
			currError = LinearRegressionImpl.error(theta, X, y);
			if(PRINT_PROGRESS){
